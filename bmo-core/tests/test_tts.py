...
# --- la voce e la clip di attesa (#21) ----------------------------------------


def test_la_voce_zittisce_la_clip_dopo_la_sintesi_e_prima_di_suonare(tmp_path):
    """La clip deve coprire anche la sintesi, che è ancora silenzio."""
    percorso = tmp_path / "frase.wav"
    scrivi_wav(percorso, PCM)
    altoparlante = AltoparlanteFinto()
    ordine = []

    def sintetizza(testo, lingua="it"):
        ordine.append("sintesi")
        return percorso

    voce = VoceTts(
        altoparlante=altoparlante,
        sintetizza_fn=sintetizza,
        diagnostica=False,
        prima_di_suonare=lambda: ordine.append(("zitto", len(altoparlante.eventi))),
    )
    voce("ciao")
    assert ordine == ["sintesi", ("zitto", 0)]
    assert altoparlante.eventi[0] == ("riproduci", percorso)


def test_anche_il_ripiego_sul_terminale_zittisce_la_clip():
    zittita = []

    def rotta(testo, lingua="it"):
        raise TtsNonDisponibile("rete giù")

    voce = VoceTts(
        altoparlante=AltoparlanteFinto(),
        ripiego=lambda testo, lingua="it": None,
        sintetizza_fn=rotta,
        diagnostica=False,
        prima_di_suonare=lambda: zittita.append(True),
    )
    voce("ciao")
    assert zittita == [True]

...
```
 ...
 ├── [CONFERMA] proponi → sì/no/boh, senza richiedere di nuovo "Hey BMO" (#17) → THINK
...
**Cosa c'è oggi al posto della wake word e del TTS.** Il diagramma sopra resta, per il resto, l'architettura di destinazione. In `bmo-core` oggi (macchina.py) il richiamo predefinito resta **Invio da tastiera** (`richiamo_da_tastiera`); la wake word **«Hey Jarvis»** (`RichiamoWakeWord`, `richiamo.py`, #22) c'è già ed è opt-in con `--wake-word`, con una soglia dichiaratamente provvisoria da tarare nella stanza vera — resta opt-in finché non è stata sentita funzionare dal vivo, sullo stesso schema di `--voce-tts`. Il modello «Hey BMO» proprio, addestrato col Colab ufficiale su dati sintetici, resta una rifinitura successiva: non serve toccare il codice per innestarlo, basta puntare `--modello-wake-word` al file `.onnx` addestrato. La voce di BMO è invece già **sintetizzata da edge-tts** (`VoceTts`, opt-in con `--voce-tts`, #42); resta **stampata sul terminale** di default (`voce_sul_terminale`, #21) finché il TTS non è stato sentito funzionare dal vivo. `scatta_foto` è dichiarato ma non ancora collegato a un esecutore: risponde sempre `non_disponibile`. Gli stati reali della macchina (`macchina.Stato`) sono ATTESA, ASCOLTO, PENSIERO, PARLATO, PAUSA, ERRORE, CONFERMA — corrispondenti a WAIT/LISTEN/THINK/SPEAK/PAUSA/ERRORE/CONFERMA qui sopra.
...
**Il diario (#14) — lettura sempre, scrittura solo confermata.** `memoria.py` rilegge `<dati>/memoria.json` a ogni turno — una lista di voci in linguaggio naturale (non coppie chiave-valore: le preferenze di casa sono troppo eterogenee per uno schema fisso), ciascuna con una data e una `fonte` ("manuale" per quelle via SCP, "modello" per quelle proposte da BMO). Il file si può sempre scrivere a mano via SSH/SCP dal computer dell'utente, esattamente come le stazioni radio preferite (§1.2bis); oltre 30 voci si tengono solo le più recenti, per non gonfiare il prompt. **Dalla #14.2, BMO può proporre lui stesso una voce nuova** con lo strumento `ricorda` (§2.4): scrive solo dopo un sì esplicito allo stato `CONFERMA` della #17, mai in silenzio.

Altri campi che compariranno qui in futuro — musica in riproduzione, fotocamera, stato della rete, l'ultima cosa detta — non esistono ancora: arriveranno con la memoria a breve termine (#29), che scriverà anche nello stesso `memoria.json` (voci con `fonte: "modello"`, estratte in automatico invece che proposte a voce).
...
Dodici strumenti (`strumenti.DICHIARAZIONI`); `imposta_espressione` **non è fra questi** — è l'etichetta a inizio risposta descritta in §2.3, non una funzione chiamabile.

**`ricorda` — propone, non scrive (#14.2).** Chiamabile sia quando viene chiesto esplicitamente ("ricordati che...") sia di iniziativa del modello, con parsimonia. L'esecuzione (`Macchina._ricorda`) chiama `chiedi_conferma` (§2.1, #17) prima di toccare il file: risponde `{"stato": "ok"}` solo dopo un sì esplicito, `{"stato": "annullato", "motivo": "rifiutato"}` per un no, o `{"stato": "annullato", "motivo": "non ho capito la conferma"}` per silenzio o ambiguità persistente. Le voci proposte da BMO hanno `fonte: "modello"` in `memoria.json` (§2.3), quelle scritte via SCP `fonte: "manuale"`.
...
**1.3 · Strumenti e stato.** Le dodici funzioni della §2.4, la macchina a stati della §2.1. *Fatto (#20): timer persistenti su disco, strumenti radio veri, macchina a stati con faccia.* *Fatto (#14.1): il diario di preferenze, in sola lettura (§2.3).* *Fatto (#17): lo stato `CONFERMA` per le conferme vocali obbligatorie, riusabile.* *Fatto (#14.2): lo strumento `ricorda`, che scrive nel diario solo dopo conferma.* Resta da fare, come issue separata: l'elenco configurabile delle persone di casa nel prompt (#15). *Il test brutale che vale più di dieci unit test*: far partire un timer, uccidere il processo, riavviarlo, verificare che suoni all'ora giusta — verificato a mano il 20/9.
